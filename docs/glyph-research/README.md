...
text screen. Photos size **width-first**, like the CLI: `FitGrid`
gives every image the full 80 columns and lets rows follow the source
aspect uncapped, so portrait content renders taller than one 25-row
screen and scrolls exactly as a terminal would show it (fox: 80×59;
the square mandrill and wheel: 80×33; synthetic patterns, which have
no native aspect, use the 80×25 screen). On a 4:3 CRT the 80-column
modes displayed cells at ~1:2.4 — CGA 640×200 (8×8 glyphs at 1:2.4
pixel aspect) and VGA 720×400 (9×16 cells) both land on exactly 1:2.4.
The modeled chain: every cell renders as its 8×8 font glyph —
...
run takes ~2.5 minutes at width-first grids).
...
ΔE 4.45, blocks+box 4.47, ASCII-only 4.95 (at ansi16: 13.24, 13.24,
14.28 — blocks+box ties the full alphabet exactly there) — alphabet
choice is an aesthetics knob, nearly free on tone; letterforms are not
a fidelity source. The hybrid converter (roadmap #1) removes smooth
regions from glyph duty entirely. Side-by-side:
...
shifts up uniformly (+2–3% at both palettes — a global linear-light
...
|---|---|---|---|
| mandrill ansi16 | 13.24 | **9.65** | 12.26 |
| mandrill ansi256 | 4.45 | **3.85** | 4.49 |
| fox ansi16 | 11.73 | **9.19** | 9.30 |
| fox ansi256 | 5.64 | **4.76** | 5.17 |
| wheel ansi16 | 13.63 | 10.67 | 9.87 |
| wheel ansi256 | 5.61 | 3.74 | 3.11 |

Display-aware matching improves the matcher 13–33% across the board —
...
dither on mandrill and fox at both palettes**, and closes the wheel
gap to 1.1–1.2×. Visual verification: the gain is real texture/color
...
[fox-ansi16](comparisons/fox-ansi16_displaymatch.png) (the 16-color
win), [fox-ansi256](comparisons/fox-ansi256_displaymatch.png),
[wheel-ansi16](comparisons/wheel-ansi16_displaymatch.png) and
[wheel-ansi256](comparisons/wheel-ansi256_displaymatch.png) (the
dither's remaining wins, kept for honesty).
...
objective with the referee is worth 11–23% to both converters
(mandrill, display-aware matcher with diffusion, blurred ΔE σ = 1
cell):

| arm / matching metric | RGB | Redmean | LAB |
|---|---|---|---|
| quadrant dither, ansi16 | — | 12.26 | 10.48 |
| display matcher, ansi16 | 9.42 | 9.65 | **8.27** |
| quadrant dither, ansi256 | — | 4.49 | 3.46 |
| display matcher, ansi256 | 3.87 | 3.85 | **3.44** |

Aligned LAB-vs-LAB, the display-aware matcher still beats the dither at
both palettes (8.27 vs 10.48; 3.44 vs 3.46 — a near-tie at 256, where
LAB matching lifts the dither most) — the headline survives the
...
~20 minutes at width-first grids, which no test can hold; it runs
through `cmd/quality` (`-methods lab -arms dither`). A `toLab`
memoization would make LAB matching practical; it is a perf item, not
a design limit.
...
| fox | 9.30 | 15.95 | 17.87 | 11.73 | 19.07 |
| mandrill | 12.26 | 15.69 | 16.26 | 13.24 | 17.95 |
| wheel | 9.87 | 23.19 | 24.74 | 13.63 | 24.89 |
...
| fox | 5.17 | 8.51 | 9.71 | 5.64 | 10.92 |
| mandrill | 4.49 | 6.25 | 6.30 | **4.45** | 7.97 |
| wheel | 3.11 | 10.38 | 11.42 | 5.61 | 11.54 |
...
- **Representation (2×2 no-diffusion vs 8×8 matcher, apples to
  ...
  and 2×2 leads by 4–12% on photos. At 256 colors they tie on four of
  six images (gray, fleshtone, color-ramp and mandrill all within 1%)
  — consistent with the original lab's conclusion that 256 colors
  close the representation gap; fox and wheel are the exceptions. (An
  earlier draft claimed the opposite by comparing the diffused dither
  against the undiffused matcher.)
- **Diffusion:** the dominant factor, worth 1.8–2.9× on ramps and
  flat-color content and 19–51% on photos, and — now measured on both
...
- **With diffusion on both sides, photos are nearly equivalent.** The
  diffused matcher *beats* the full quadrant dither outright on
  mandrill at 256 colors (4.45 vs 4.49) and on the 16-color gray
  gradient (2.33 vs 2.53 — finer spatial masks pay off when the
  palette is coarse), and lands within 1% on fleshtone (4.10 vs 4.08).
  The real gaps that remain are fox-256 (5.64 vs 5.17) and the
  saturated flats — color-ramp (4.76 vs 3.24) and wheel (5.61 vs 3.11)
  — where 2 px cells simply track hue boundaries better than 8 px
  cells.
